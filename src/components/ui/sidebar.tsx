...
function Sidebar({
  ...
  // Compute widths directly from React state — avoids relying on CSS custom
  // property utilities (w-(--sidebar-width)) that may not resolve when the
  // variable isn't registered in @theme.
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* Spacer — reserves width in the flex layout so SidebarInset is pushed right */}
      <div
        className={cn(
      ...
      />
      {/* Fixed panel — sits on top of the spacer area */}
      <div
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh transition-[left,right,width] duration-200 ease-linear md:flex",
          side === "left"
            ? isOffcanvasCollapse ? "-left-64" : "left-0"
            : isOffcanvasCollapse ? "-right-64" : "right-0",
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "bg-background relative flex min-h-screen flex-1 flex-col overflow-hidden",
        className
      )}
      {...props}
    />
